...
export class AuthService {
    ...
    async login(email: string, password: string) {
        ...
        if (!user) {
            throw new HttpException('User with this email does not exist in system', HttpStatus.BAD_REQUEST);
        }
        ...
        const token = await this.tokenService.createToken({ author: user, token: accessToken });
